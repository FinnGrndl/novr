...
2. Download the latest installer from the [NOVR releases page](https://github.com/FinnGrndl/novr/releases/latest):
...
4. Choose the NOVR release you want to install.
5. If Nuclear Option is not found automatically, choose the game folder manually.
    - The selected folder must contain `NuclearOption_Data/Managed`.
6. Click **Install**.
7. Launch Nuclear Option from Steam.
...
- Downloads the selected NOVR release zip from `FinnGrndl/novr`.
- Installs NOVR into:
...
3. Download `NOVR.zip` from the [latest NOVR release](https://github.com/FinnGrndl/novr/releases/latest).
...
This fork publishes from the `release` branch. Push a version bump to `release` and GitHub Actions will:

- Build `dist/NOVR.zip`.
- Build `dist/NOVR.Installer-Win.exe` and `dist/NOVR.Installer-Linux`.
...
GitHub Pages is generated by `.github/workflows/pages.yml` using `scripts/generate-pages.sh`. Enable Pages with **Build and deployment > Source: GitHub Actions** in the repository settings. The generated site summarizes the build pipeline, installer flow, and BepInEx runtime layout.
